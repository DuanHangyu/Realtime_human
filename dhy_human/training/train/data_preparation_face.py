...
def run(video_path, export_imgs = True):
    ...
    if export_imgs:
        ...
        crop_w = right_coincidence - left_coincidence
        crop_h = bottom_coincidence - top_coincidence
        subprocess.run([
            "ffmpeg", "-i", video_path,
            "-vf", "crop={}:{}:{}:{},scale=512:512:flags=neighbor".format(crop_w, crop_h, left_coincidence, top_coincidence),
            "-loglevel", "quiet", "-y", "{}/image/%06d.png".format(video_data_path)
        ], check=False)
