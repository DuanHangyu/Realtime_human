...
def check_step0(task_id, video_path):
    try:
        ...
        if scale > 1:
            ...
            subprocess.run(["ffmpeg", "-i", video_path, "-r", "25", "-ss", "00:00:00", "-t", "00:02:00", "-vf", "scale={}:{}".format(new_width, new_height), "-an", "-loglevel", "quiet", "-y", front_video_path], check=False)
        else:
            subprocess.run(["ffmpeg", "-i", video_path, "-r", "25", "-ss", "00:00:00", "-t", "00:02:00", "-an", "-loglevel", "quiet", "-y", front_video_path], check=False)
...
def check_step1(task_id):
    ...
    if ExtractFromVideo(task_id, front_video_path) != 1:
        ...
        return 0
    subprocess.run(["ffmpeg", "-i", front_video_path, "-vf", "reverse", "-loglevel", "quiet", "-y", back_video_path], check=False)
    subprocess.run(["ffmpeg", "-f", "concat", "-i", "data/video_concat.txt", "-loglevel", "quiet", "-y", video_out_path], check=False)
    ...
    subprocess.run(["ffmpeg", "-i", front_video_path, "-vf", "crop=w='min(iw\\,ih)':h='min(iw\\,ih)',scale=256:256,setsar=1", "-vframes", "1", preview_path], check=False)
...
def check_step0_audio(task_id, video_path):
    ...
    subprocess.run(["ffmpeg", "-i", video_path, "-ac", "1", "-ar", "16000", "-loglevel", "quiet", "-y", wav_path], check=False)
